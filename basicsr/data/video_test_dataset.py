...
class VideoTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt['name'].lower() in ['vid4', 'reds4', 'redsofficial','underwater_video_sr_val']:
            for subfolder_lq, subfolder_gt in zip(subfolders_lq,
                ...
                stride = self.opt['num_frame']
                
                # Step by 'stride' to create non-overlapping chunks
                for i in range(0, max_idx, stride):
                    self.data_info['lq_path'].append(img_paths_lq[i])
                    self.data_info['gt_path'].append(img_paths_gt[i])
                    self.data_info['folder'].append(subfolder_name)
                    self.data_info['idx'].append(f'{i}/{max_idx}')
                    self.data_info['border'].append(0) # Borders matter less for non-overlapping
    ...
    def __getitem__(self, index):
        ...
        # -------------------------------------------------------------------------
        # MODIFIED: Logic to handle EVEN frame counts (e.g., 8) for VideoSRNet
        # -------------------------------------------------------------------------
        num_frame = self.opt['num_frame']
        
        if num_frame % 2 == 1:
            # Standard BasicSR behavior for ODD frames (Center-oriented)
            select_idx = generate_frame_indices(
                idx, max_idx, num_frame, padding=self.opt['padding'])
        else:
            # Custom behavior for EVEN frames (e.g., 8)
            # Strategy: Take [idx, idx+1, ..., idx+7]
            select_idx = list(range(idx, idx + num_frame))
            
            # Fetch the padding mode from the YAML config (defaulting to replicate)
            padding_mode = self.opt.get('padding', 'replicate').lower()
            
            for i in range(len(select_idx)):
                if select_idx[i] >= max_idx:
                    if padding_mode in ['replicate', 'repeat']:
                        # Clamp to the last frame
                        select_idx[i] = max_idx - 1
                    elif padding_mode in ['reflection', 'reflect']:
                        # Bounce back
                        overhang = select_idx[i] - (max_idx - 1)
                        select_idx[i] = max(0, (max_idx - 1) - overhang)
                    elif padding_mode in ['circular', 'wrap']:
                        # Loop to the start
                        select_idx[i] = select_idx[i] % max_idx
                    else:
                    ...
                    if padding_mode in ['replicate', 'repeat']:
                        ...
                        select_idx[i] = min(max_idx - 1, -select_idx[i])
                    elif padding_mode in ['circular', 'wrap']:
                        select_idx[i] = select_idx[i] % max_idx
                    else:
                        raise ValueError(f"Padding mode '{padding_mode}' is not supported.")
        ...
        if self.cache_data:
            ...
            # Load sequence for GT (matches model output shape)
