...
# Initialize globally to avoid reloading weights for every image/frame
loss_fn_alex = None

def calculate_lpips(img1, img2, crop_border, input_order='HWC', **kwargs):
    """Calculate LPIPS (Learned Perceptual Image Patch Similarity)."""
    global loss_fn_alex
    if loss_fn_alex is None:
        loss_fn_alex = lpips.LPIPS(net='alex').cuda()

    # Handle PyTorch tensors if passed directly (similar to existing PSNR/SSIM functions)
    if type(img1) == torch.Tensor:
        if len(img1.shape) == 4:
            img1 = img1.squeeze(0)
        img1 = img1.detach().cpu().numpy().transpose(1, 2, 0)
    ...
    # Convert numpy arrays (0-255) to PyTorch Tensors (0-1)
    ...
    # Normalize to [-1, 1] as expected by the LPIPS model
    img1_t = img1_t * 2.0 - 1.0
    img2_t = img2_t * 2.0 - 1.0

    with torch.no_grad():
        lpips_val = loss_fn_alex(img1_t.cuda(), img2_t.cuda())
